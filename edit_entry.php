...
if (isset($_GET['state'])) {
    ...
    $temp_url = "http://$_SERVER[HTTP_HOST]" . "/edit_entry.php?" . $state_values . "&test=post-state";
    $state_values = null;
    $temp = null;
...
if (!isset($_GET['code'])) {
    $state = str_replace("/booking/edit_entry.php?", "", $state);
    $state = str_replace("edit_entry.php?", "", $state);
    $state = base64_encode($state);
}

$redirect_uri = "http://" . $_SERVER[HTTP_HOST] . "/edit_entry.php";
...
$multiday_allowed = false;
// Similarly for multiple room selection
$multiroom_allowed = true;
...
<!--START SIDEBAR CODE-->
<div id="sidebar" style="float:right; width:400px; background-color:#FAA; padding:10px;">
    <strong>IMPORTANT</strong><br /><br />
    The Common Room is available for undergraduate engineering McGill students for free, but charges will apply if there are damages or if cleaning and/or room organization is required after the event. <br /><br />
    The Clubs Hub is available only to EUS Clubs, so your booking will be rejected if you do not book using a Google account ending in *@mcgilleus.ca. <br /><br />
    The Trottier 5th Floor Study Rooms are available to all McGill Engineering students, but priority is given to Electrical, Computer and Software Engineering students. Bookings are a maximum of 2 hours per day, per student. Please put your full name and major in the description box. Failure to do so will result in a rejected booking. <br /><br />
    Please indicate in the description if you:<br />
    <ul>
        <li>will be serving alcohol</li>
        <li>require audio</li>
        <li>need a projector, music, and/or the kitchen</li>
        <li>are not an Engineering student</li>
    </ul>
...
foreach ($edit_entry_field_order as $key)
{
  ...
  default:
      if ($key != "Email") {
          ...
          if (!isset($userEmail)) {
              $userEmail = "Something went wrong. Please contact the IT Director if you see this at: it.director@mcgilleus.ca";
              //heya IT Director, you may see this because the error above showed, spend some time looking
              //through the code here to discover the source of the error! Ask Lou Bernardi if you have any
              //questions, remember: it's dangerous to go alone!
          }
          echo "<div><p><b>Currently logged in as:</b> <span style=\"color:blue;\">" . $userEmail . "</span></p></div>";
          echo "<input type=\"hidden\" name=\"f_Email\" value=\"" . $userEmail . "\"";
          echo "<br><br>";
      }
    break;
  }
