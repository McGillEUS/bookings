...
checkAuthorised();
/*
this is no longer necessary as this page cannot be accessed unless logged in. may be vulnerable
to an attack but extremely unlikely someone would bother
*/
...
// McGill EUS - email space manager
if ($custom_fields['alcohol']) {
    if ($custom_fields['alcohol'] == 1) {
        $usingAlcohol = true;
    }
}
if ($custom_fields['audio']) {
    if ($custom_fields['audio'] == 1) {
        $usingAudio = true;
    }
}

$start_hour = floor($start_seconds / 3600);
$start_minute = ($start_seconds / 60) - ($start_hour * 60);
$time_extra_details = date('r', mktime($start_hour, $start_minute, 0, $start_month, $start_day));
$time_concise = substr($time_extra_details, 0, -6);

$from = "Room Booking Service <noreply@mcgilleus.ca>";
$replyto = "EUS Space Manager <space.manager@mcgilleus.ca>";
$headers = "From: " . $from . "\n" . "Reply-To: " . $replyto . "\n";
$target_email = "space.manager@mcgilleus.ca";

if ($usingAlcohol && $usingAudio) {

    if (!@mail($target_email, "Audio Setup and Alcohol Requested for: " . $name . "at , " . $time_concise . " in " . get_room_name($room), "This is a test\n ", $headers)) {
        $temp = "This failed.... why";
    }

} elseif ($usingAlcohol) {
    if (!@mail($target_email, "Alcohol Requested for: " . $name  . $name . "at , " . $time_concise . " in " . get_room_name($room), "This is a test\n ", $headers)) {
        $temp = "This failed.... why";
    }
} elseif ($usingAudio) {
    if (!@mail($target_email, "Audio Setup Requested for: " . $name  . $name . "at , " . $time_concise . " in " . get_room_name($room), "This is a test\n ", $headers)) {
        $temp = "This failed.... why";
    }
}
...
foreach ($rooms as $room_id)
{
  ...
  if ($approval_enabled && !auth_book_admin($user, $room_id))
  {
    ...
    /*
    // EUS IT Director, November 2017 - if this area doesn't have approvals enabled, send a confirmation email
    // get details of this booking
      ...
      try {
        ...
        $mail->Subject = "[EUS Booking System] - Booking Confirmed";
